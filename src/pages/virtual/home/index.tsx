...
import { Droplet, Skull, Thermometer, Router } from "lucide-react";
...
export default function HomePage() {
  ...
  const sensorIconMap: Record<string, React.ReactElement> = {
    ...
    tvoc: <Skull className="text-red-500 text-sm h-4 w-4" />, // 有害气体
    coo: <CloudOutlined className="text-red-500 text-sm" />, // 二氧化碳
    waterleakage: <Droplet className="text-red-500 text-sm h-4 w-4" />,
  };
  ...
  function getSensorIcon(content: string) {
    if (content.includes("终端及其传感器掉线")) {
      return <Router className="w-4 h-4 text-red-500 text-sm" />;
    }
          ...
          >
            <div className="flex flex-col items-center justify-center gap-4 p-8">
              <div className="flex items-center justify-center w-20 h-20 bg-gray-200 rounded-full">
                <CircleSlash size={40} className="text-gray-500" />
              </div>
              <h1 className="text-2xl font-bold text-gray-800">暂无权限</h1>
                  ...
                  <div
                    ...
                    style={{
                      width: (() => {
                        ...
                        // 以昨日的值作为基准
                        const width =
                          (device_unit.count / yesterdayCount) * 100;
                        // 限制在 0~100%
